package com.hiwifi.utils.calendar;
...
//    final static String chineseNumber[] = {"一", "二", "三", "四", "五", "六", "七", "八", "九", "十", "十一", "十二"};
     ...
     * @return  
     */  
    public NongliUtil(Calendar cal) {  
    ...
    public String GetMonthDate(){
    	if(month - 1 == 0){
    		return (leap ? "闰" : "") + "正月" + getChinaDayString(day); 
    	}
    ...
    public String getNongliDateString(){
    	if(month - 1 == 0){
    		return cyclical()+"年"+(leap ? "闰" : "") + "正月" + getChinaDayString(day); 
    	}
    ...
    public  Date getNongliDate() {
        DateFormat format = new SimpleDateFormat("yyyy-MM-dd");
        Date nongliDate = null;

        try {
        	nongliDate = format.parse(year+"-"+month+"-"+day);
        } catch (Exception e) {
           HWFLog.e("Nongli", "get nongli date error", e);
        }
