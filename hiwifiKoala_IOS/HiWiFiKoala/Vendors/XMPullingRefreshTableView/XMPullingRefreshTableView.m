...
- (id)initWithFrame:(CGRect)frame atTop:(BOOL)top {
    ...
    if (self) {
        ...
        _dateLabel.text = NSLocalizedString(@"最后更新:刚刚", @"");
        ...
        _logoView.center = _arrowView.center;
        _logoView.tintColor = [UIColor colorWithRed:0.132 green:0.628 blue:0.897 alpha:1.000];
        _logoView.lineWidth = 0.5;
...
//        dateFrame = CGRectMake(0, y, size.width, kPRLabelHeight);
        ...
        y = margin;
//        arrowFrame = CGRectMake(4*x, y, kPRArrowWidth, kPRArrowHeight);
        arrowFrame = CGRectMake((size.width-kPRArrowWidth)/2, y, kPRArrowWidth, kPRArrowHeight);
...
- (void)refreshLogoViewStrokeEnd:(CGFloat)strokeEnd
{
    [_logoView refreshStrokeEnd:strokeEnd];
}

- (void)setState:(PRStatue)state {
    [self setState:state animated:YES];
}

- (void)setState:(PRStatue)state animated:(BOOL)animated{
...
@interface XMPullingRefreshTableView ()
- (void)scrollToNextPage;
@end

@implementation XMPullingRefreshTableView
...
- (id)initWithFrame:(CGRect)frame pullingDelegate:(id<XMPullingRefreshTableViewDelegate>)aPullingDelegate {
...
- (void)tableViewDidScroll:(UIScrollView *)scrollView {
    // 下拉
    [_headerView refreshLogoViewStrokeEnd:-scrollView.contentOffset.y/kPROffsetY];
